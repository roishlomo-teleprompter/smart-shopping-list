...
const MainList: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const unsubList = onSnapshot(listRef, (snap) => {
      if (snap.exists()) setList(snap.data() as ShoppingList);
    });
    ...
    <div className="flex flex-col min-h-screen max-w-md mx-auto bg-slate-50 relative pb-32 shadow-2xl overflow-hidden dir-rtl" dir="rtl">
      ...
      <header className="sticky top-0 z-40 bg-white/80 backdrop-blur-md px-6 py-4 flex items-center justify-between border-b border-slate-100">
        <div className="flex items-center gap-2">
           <button onClick={getAiSuggestions} className={`p-2 rounded-full ${isAiLoading ? 'bg-indigo-100 animate-pulse' : 'bg-slate-100 hover:bg-indigo-50 text-indigo-600'}`}>
                  ...
                  {activeItems.map(item => (
                    <div key={item.id} className="flex items-center justify-between p-3 bg-white rounded-2xl border border-slate-100 shadow-sm transition-all hover:border-indigo-100">
                      <div className="flex items-center gap-1">
                        ...
                        <button onClick={() => toggleFavorite(item.id)} className={`p-2 transition-colors ${item.isFavorite ? 'text-amber-500' : 'text-slate-300'}`}>
      ...
      {activeTab === 'list' && activeItems.length > 0 && (
        <button onClick={shareWhatsApp} className="fixed bottom-28 left-6 right-6 bg-emerald-500 text-white py-4 rounded-2xl shadow-xl shadow-emerald-100 flex items-center justify-center gap-2 font-black z-30 active:scale-95 transition-all border-2 border-white">
      ...
      {showClearConfirm && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 bg-slate-900/40 backdrop-blur-sm">
          <div className="bg-white rounded-3xl w-full max-w-xs p-8 shadow-2xl text-center space-y-6">
            <div className="mx-auto w-16 h-16 bg-rose-50 text-rose-500 rounded-full flex items-center justify-center"><AlertCircle className="w-8 h-8" /></div>
            <div>
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <button onClick={clearList} className="py-4 rounded-2xl bg-rose-500 text-white font-bold">מחיקה</button>
            </div>
          </div>
        </div>
      )}
